...
class PveClient {
  ...
  /**
   * Start a vzdump backup of a guest. Returns the UPID.
   * @param {object} opts { storage, mode='snapshot', compress='zstd', notes }
   */
  runBackup(node, vmid, {
    storage, mode = 'snapshot', compress = 'zstd', notes,
  } = {}) {
    if (!storage) throw new PveError('Missing backup storage');
    const body = {
      vmid, storage, mode, compress,
    };
    if (notes) body['notes-template'] = notes;
    return this.post(`/nodes/${encodeURIComponent(node)}/vzdump`, body);
  }
