...
class DiskBalancer {
  ...
  constructor(homey, connectionManager) {
    ...
    this._running = false;
    this._onSettingsSet = (key) => this._onSet(key);
  }

  /**
   * React to app-settings changes. Saving the settings page fires one `set`
   * event per field, so debounce them into a single reschedule.
   */
  _onSet(key) {
    if (key && !String(key).startsWith('balance') && key !== 'autoBalanceEnabled') return;
    if (this._rescheduleTimer) this.homey.clearTimeout(this._rescheduleTimer);
    this._rescheduleTimer = this.homey.setTimeout(() => {
      this._rescheduleTimer = null;
      this._reschedule();
  ...
  stop() {
    ...
    if (this._rescheduleTimer) { this.homey.clearTimeout(this._rescheduleTimer); this._rescheduleTimer = null; }
    this._clearTimers();
  }
  ...
  /**
   * @param {boolean} force run even when auto-balancing is disabled
   *   (used by the "Run balancer now" Flow action). The dry-run setting is
   *   always honoured.
   */
  async runOnce(force = false) {
    const cfg = this._settings();
    if ((!cfg.enabled && !force) || this._running) return;
    this._running = true;
    this.homey.app.log(`[balancer] run started (dryRun=${cfg.dryRun}${force ? ', manual' : ''}, high=${cfg.highPct}%, low=${cfg.lowPct}%)`);
    try {
      const connections = [...this.connections.connections.values()];
      if (!connections.length) {
        this.homey.app.log('[balancer] no Proxmox connections yet — pair a device first');
      }
      ...
      this._running = false;
      this.homey.app.log('[balancer] run finished');
    }
  }
  ...
  async _balanceNode(client, node, cfg, typeByVmid) {
    ...
    if (!eligible.length) {
      this.homey.app.log(`[balancer] ${node}: no image-capable storages found`);
      return;
    }

    const fmt = (s) => {
      const excluded = NETWORK_STORAGE_TYPES.has(s.type) && !cfg.includeNetwork;
      const listed = cfg.excludeStorages.includes(s.storage);
      const tag = (excluded || listed) ? ' [target-excluded]' : '';
      return `${s.storage} (${s.type || '?'}) ${Math.round(s.pct)}%${tag}`;
    };
    const usage = eligible.map(fmt).join(', ');

    const source = eligible
      .filter((s) => s.pct >= cfg.highPct && Number(s.used) > 0)
      .sort((a, b) => b.pct - a.pct)[0];
    if (!source) {
      this.homey.app.log(`[balancer] ${node}: nothing above ${cfg.highPct}% — ${usage}`);
      return;
    }
    this.homey.app.log(`[balancer] ${node}: ${source.storage} at ${Math.round(source.pct)}% is over threshold — ${usage}`);
