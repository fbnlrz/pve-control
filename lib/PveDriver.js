...
class PveDriver extends Homey.Driver {
  ...
  _registerGuestFlow(prefix) {
    ...
    this._registerBackupFlow(prefix);
  }

  _registerBackupFlow(prefix) {
    const card = this._action(`${prefix}_backup`, async (args) => {
      const d = args.device;
      const storage = args.storage && args.storage.storage;
      if (!storage) throw new Error('Select a backup storage');
      await d.client.runBackup(d.pveNode, d.vmid, { storage, mode: args.mode || 'snapshot' });
      return true;
    });
    if (!card) return;

    card.registerArgumentAutocompleteListener('storage', async (query, args) => {
      const d = args.device;
      const storages = await d.client.getNodeStorages(d.pveNode, 'backup');
      const q = (query || '').toLowerCase();
      return (storages || [])
        .filter((s) => s.enabled !== 0 && s.active !== 0)
        .filter((s) => storageHasContent(s, 'backup'))
