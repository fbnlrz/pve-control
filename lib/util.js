...
/**
 * Convert a byte-rate into MB/s (decimal megabytes, as used for disk I/O).
 */
function bytesToMbyte(bytesPerSecond, decimals = 2) {
  if (bytesPerSecond === null || bytesPerSecond === undefined) return null;
  return round(Number(bytesPerSecond) / 1e6, decimals);
}
...
/**
 * Extract the first usable non-loopback IPv4 address from a guest's network
 * interface data.
 * @param {'qemu'|'lxc'} kind
 * @param {object|Array} data QEMU agent result ({result:[...]}) or LXC iface array
 */
function extractGuestIp(kind, data) {
  if (!data) return null;
  if (kind === 'qemu') {
    const ifaces = Array.isArray(data.result) ? data.result : (Array.isArray(data) ? data : []);
    for (const iface of ifaces) {
      if (iface.name === 'lo') continue;
      for (const addr of iface['ip-addresses'] || []) {
        const ip = addr['ip-address'];
        const isV4 = addr['ip-address-type'] === 'ipv4' || (ip && ip.includes('.'));
        if (isV4 && ip && !ip.startsWith('127.')) return ip;
      }
    }
    return null;
  }
  ...
  for (const iface of ifaces) {
    if (iface.name === 'lo') continue;
    if (iface.inet && !String(iface.inet).startsWith('127.')) {
      return String(iface.inet).split('/')[0];
    }
  }
  return null;
}
...
/**
 * Best-effort static IP from an LXC config. Container net interfaces look like
 * "name=eth0,bridge=vmbr0,ip=192.168.1.50/24,gw=..." — returns the first
 * non-DHCP IPv4 found across net0..netN.
 */
function parseLxcConfigIp(config) {
  if (!config) return null;
  for (const [key, value] of Object.entries(config)) {
    if (!/^net\d+$/.test(key) || typeof value !== 'string') continue;
    const match = value.match(/(?:^|,)ip=([^,]+)/);
    if (!match) continue;
    const ip = match[1].split('/')[0];
    if (ip && ip !== 'dhcp' && ip !== 'manual' && !ip.startsWith('127.')) return ip;
  }
  return null;
}
